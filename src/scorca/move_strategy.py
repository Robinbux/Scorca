...
POLICY_INDICES_WITH_MOVES = read_dict_from_file('/Users/robinbux/Desktop/Scorca_clean/src/scorca/policy_indices.json')
POLICY_INDICES = [int(index) for index in list(POLICY_INDICES_WITH_MOVES.keys())]
MOVES = list(POLICY_INDICES_WITH_MOVES.values())
...
class MoveStrategy:
    ...
    def get_move_weights_and_move_counts(self, boards: List[chess.Board], possible_moves: List[chess.Move],
        ...
        # Get the indeces of the possible moves
        # possible_moves_uci = [move.uci() for move in possible_moves]
        # if '0000' in possible_moves_uci:
        #     possible_moves_uci.remove('0000')
        #
        # filtered_policy_indices_with_moves = {k: v for k, v in POLICY_INDICES_WITH_MOVES.items() if
        #                                       v in possible_moves_uci}
        # filtered_policy_indices = [int(index) for index in list(filtered_policy_indices_with_moves.keys())]
        # filtered_moves = list(filtered_policy_indices_with_moves.values())
    ...
    def calculate_move_weights_and_get_likely_boards(board: chess.Board, b, possible_moves: List[chess.Move],
        ...
        moves = game_state.moves()
        #new_indices = [MOVE_TO_IDX[move] for move in moves]
        policy_indices = game_state.policy_indices()
        #move_scores = list(zip(moves, eval.p_raw(*policy_indices)))  # list of (move, probability) tuples
        move_scores_softmax = list(zip(moves, eval.p_softmax(*policy_indices)))
        # move_scores_softmax = list(zip(filtered_moves, eval.p_softmax(*filtered_policy_indices)))
...
#     res = [0] * (4 * 64 * 64)
#     for i in range(len(IDX_TO_MOVE)):
#         res[IDX_TO_MOVE[i].as_packed_int()] = i
#     return res
#
# MOVE_TO_IDX = build_move_indices()
