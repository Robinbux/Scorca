        ...
        with Pool(processes=self.num_cpus) as pool:
            for board in self.possible_states:
                if time.time() - start_time >= seconds_left:
                    ...
                    break  # stops the computation if the time limit has been reached
                # if len(self.possible_states) < 100:
                #     best_n_moves = get_best_n_moves(board, n=5, seconds=1, stockfish_engine=stockfish_engine)
                #     # For each move, check if it is possible with captured_my_piece and capture_square
                ...
                #             continue
                #         self.best_squares_for_opp_to_move_to.update([move.to_square])
